...
- Follow **snake_case** for variables and functions. File names follow the language convention: `snake_case` for
  Python, `kebab-case` for TypeScript modules (`get-findings.ts`), `PascalCase` for React components.
...
- `main` — always deployable, tagged at the end of each sprint (e.g. `v.1`, `v.2`...)
...
3. Open PR into `develop` and fill in the PR template (`.github/pull_request_template.md`); CI must pass (lint + tests + secret scan)
